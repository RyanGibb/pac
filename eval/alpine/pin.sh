#!/usr/bin/env bash
# Re-ask pac every goal of a scale.sh run that diverged from apk, with the
# provider apk chose named in our world as well.  A goal that then agrees
...
ok=0; bad=0
set -f
for k in $(sed -n 's/^goal=\([^ ]*\) .* corr=diff .*/\1/p' "$run/results.txt"); do
