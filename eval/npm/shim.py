...
/<name> returns that file unchanged; nothing else is served, and with
--frozen a name the snapshot does not hold is a 404 rather than a fetch,
which is what keeps a measured run off the live registry.
...
`npm install --package-lock-only` never asks for a tarball, so there is
no /<name>/-/<file>.tgz route here; dist.tarball in the served document
still points at registry.npmjs.org and is only ever copied into the
lockfile.
...
def fetch(name, path):
    ...
    url = "https://registry.npmjs.org/" + urllib.parse.quote(escape(name), safe="%@.-_")
        ...
        if not os.path.exists(path):
            if LOG:
                with open(LOG, "a") as f:
                    f.write(name + "\n")
            if not (FILL and fetch(name, path)):
                self.send_response(404)
                self.end_headers()
                return
