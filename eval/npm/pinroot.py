...
Because the block goes into the one package.json both sides are handed,
the two are still asked the same question.  If the answers then coincide,
npm's resolution was one our instance already admitted and only our
preference ordering ranked it second.  If our side is unsatisfiable or
still differs, the instance itself differs.

usage: pinroot.py <package.json> <name@version>...
"""
import json
import sys
